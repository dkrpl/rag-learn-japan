...
from app.models.simulation import JlptSimulation, JlptSimulationSection, JlptSimulationQuestion, UserSimulationAttempt, UserSimulationAttemptSection
...
def test_simulation_timer_expired(client: TestClient, db: Session, learner_token_headers: dict):
    ...
    attempt_sec = db.query(UserSimulationAttemptSection).filter(UserSimulationAttemptSection.attempt_id == attempt_id).first()
